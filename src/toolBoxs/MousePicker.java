...
public class MousePicker {
    ...
    public MousePicker(Camera cam, Matrix4f projection, List<NormaledTerrain> terrains) {
        camera = cam;
    ...
    private Vector3f binarySearch(int count, float start, float finish, Vector3f ray) {
        ...
        if (count >= RECURSION_COUNT) {
            ...
            NormaledTerrain terrain = getTerrainIndex(endPoint.getX(), endPoint.getZ());
            if (terrain != null) {
                return endPoint;
            } else {
                return null;
    ...
    private boolean isUnderGround(Vector3f testPoint) {
        NormaledTerrain terrain = getTerrainIndex(testPoint.getX(), testPoint.getZ());
        float height = 0;
